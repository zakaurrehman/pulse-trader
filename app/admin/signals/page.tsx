...
export default function AdminSignalsPage() {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="mb-8">
        <h1 className="text-2xl font-black text-ink">Signal Performance Stats</h1>
      ...
      <div className="space-y-6">
        {stats.map((stat) => (
          <div key={stat.id} className="bg-panel rounded-2xl border border-hairline shadow-sm p-6">
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 ${PAIR_COLORS[stat.pair] ?? "bg-fog"} rounded-xl flex items-center justify-center`}>
                  <span className="text-void font-black text-xs">{stat.pair.split("/")[0]}</span>
                </div>
                <h2 className="text-ink font-bold text-lg">{stat.pair}</h2>
              </div>
              {message?.id === stat.id && (
                <span className={`text-xs font-semibold px-3 py-1 rounded-full ${
                  message.ok ? "bg-green-500/15 text-green-400" : "bg-red-500/15 text-red-400"
                }`}>
                  {message.text}
                </span>
              )}
            ...
            <div className="flex items-end gap-1 h-8 mb-5 bg-abyss rounded-lg px-3 py-1">
            ...
            <button
              ...
              className="bg-gold-500 hover:bg-gold-highlight disabled:opacity-50 text-void font-bold text-sm px-6 py-2.5 rounded-lg transition-colors"
            >
        ...
        {stats.length === 0 && (
          <div className="text-center py-12 text-fog">Loading signal stats…</div>
        )}
      </div>
    </div>
  );
